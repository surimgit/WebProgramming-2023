...
class Thread2 implements Runnable{
	// Runnable 인터페이스의 추상메소드 run()을 구현
	@Override
	public void run() {
		for(int i = 0; i < 20; i++) {
			System.out.print("#");
	...
	public void run() {
		...
		for (int i = 10; i > 0; i--) {
			...
			try {
				...
				// Blocked 상태: 실행중이지만 CPU 스켸줄러에 선택은 받지 못하는 상태
				Thread.sleep(1000);
			} catch (Exception e) {
				e.printStackTrace();
			}
...
public class Example_06 {
	public static void main(String[] args) {
		...
		// Thread 클래스의 생성자의 인자로 담아서 Thread 인스턴스를 생성한 후
		// 사용가능
		Thread2 thread2 = new Thread2();
		Thread thread = new Thread(thread2);
		
		// Thread 클래스의 setPriority 메소드로 우선순위를 지정
		// 우선순위를 지정하지 않으면 기본 값으로 5
		thread1.setPriority(1);
		thread.setPriority(10);
		// 상속받은 Thread클래스의 start()메소드로 해당 Thread를 실행
