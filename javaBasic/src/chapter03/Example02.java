...
// 1. 접근 제어자
// -public protect, default(package), private
// 2. 일반 제어자
...
public abstract class Example02 {
	...
	// static, final
	// static 제어자: 해당 멤버변수와 메소드를 클래스 단위로 관리하기 위한 용도
	// final 제어자: 초기화 이후 값을 변형시키지 않고자 할 때 사용
	static final int NUMBER_5 = 10;
	
	// abstract 제어자
	// 메소드를 선언부만 작성하고 실제 구현부는 상속받는 클래스가 구현하고자 할때
	// 클래스, 메소드에서만 사용가능
	...
	// 제어자를 조합할 때 주의할 사항
	// 메소드에 static과 abstract를 동시에 사용할 수 없음
	// 클래스에 final과 abstract를 동시에 사용할 수 없음
	// abstract 메소드를 접근제어자는 private일 수 없음
	// final 과 private은 동시에 사용할 필요가 없음
	
	
	public static void main(String[] args) {
		
		// Modifier 클래스의 경우 public으로 선언되었기 때문에 모든 위치에서 사용 가능
