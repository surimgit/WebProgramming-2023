...
// 데이터베이스에 접근해서 검색 및 삽입 등의 데이터베이스 작업을 담당하는 클래스
public class BoardDAO {

	// 데이터베이스의 Board 테이블에서 입력받은 id에 해당하는 레코드를 모든 컬럼을 선택해서 반환하는 메소드
	// SQL: SELECT *FROM Board WHERE id = ?;
	// 예상 반환 결과: 0개 or 1개의 레코드가 반환
	...
	// 데이터베이스의 Board 테이블에서 모든 컬럼을 선택해서 반환하는 메소드
	// SQL: SELECT *FROM Board;
	// 예상되는 반환결과는 0개 이상의 복수의 레코드
	...
	// 데이터베이스에서 Board테이블의 레코드중 입력받은 ID에 해당하는 레코드의 
